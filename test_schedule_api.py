...
def base_info():
    ...
    return {
        ...
        "lesson_Types": [{"name": "Лекция", "id": "lt-lec"}, {"name": "Практическая работа", "id": "lt-prac"}],
        "audithories": [
...
def schedule_payload():
    ...
    ts = int(datetime(2026, 9, 12, tzinfo=SCHEDULE_TZ).timestamp() * 1000)
    return {
        "success": True,
        "schedule": {
            "error": "",
...
def test_build_lookup_handles_empty_base():
    lookup = build_lookup({})
    for key in ("divisions", "groups", "teachers", "disciplines", "lesson_types", "auditoria", "territories"):
        assert lookup[key] == {}
...
def test_fetch_base_info_success(base_info):
    with patch("schedule_api.requests.get") as mock_get:
        ...
        mock_response.json.return_value = {"success": True, "baseInfo": {"data": base_info}}
...
def test_fetch_base_info_retries_on_503():
    with patch("schedule_api.requests.get") as mock_get, patch("schedule_api.time.sleep") as mock_sleep:
...
def test_fetch_base_info_gives_up_after_max_retries():
    with patch("schedule_api.requests.get") as mock_get, patch("schedule_api.time.sleep"):
...
def test_fetch_schedule_sends_correct_timestamp(schedule_payload):
    with patch("schedule_api.requests.get") as mock_get:
        ...
        expected_ts = int(datetime(2026, 9, 12, tzinfo=SCHEDULE_TZ).timestamp() * 1000)
...
def test_get_group_lessons_returns_only_target_group(base_info, schedule_payload):
    with patch("schedule_api.requests.get") as mock_get:
        mock_response = MagicMock(status_code=200)
        mock_response.json.return_value = schedule_payload
        mock_get.return_value = mock_response
...
def test_get_group_lessons_empty_when_group_absent(base_info, schedule_payload):
    with patch("schedule_api.requests.get") as mock_get:
        mock_response = MagicMock(status_code=200)
        mock_response.json.return_value = schedule_payload
        mock_get.return_value = mock_response
...
def test_get_group_lessons_empty_when_date_mismatch(base_info, schedule_payload):
...
def test_get_week_lessons_tolerates_individual_failures(base_info):
    ...
    def fake_get(url, params=None, headers=None, timeout=None):
        ...
        ts_target = int(datetime(2026, 9, 9, tzinfo=SCHEDULE_TZ).timestamp() * 1000)
        if params and params.get("date") == ts_target:
            raise ScheduleAPIError("boom")
        resp = MagicMock(status_code=200)
        resp.json.return_value = {"success": True, "schedule": {"data": {"schedule": []}}}
        return resp
...
@pytest.mark.parametrize("user_input", ["СП-4", "СП4", "сп-4", "сп4", "(СП-4)", "  СП-4  ", "CП 4"])
def test_find_group_division_normalization(base_info, user_input):
    ...
    base = dict(base_info)
    base["divisions"] = [{"name": "(СП-4) Энергетическое отделение", "id": "div-sp4"}]
    base["groups"] = [{"name": "419", "id": "grp-419", "division": "div-sp4", "curse": 3}]
